...
public class ContaCorrente {
    ...
    public boolean sacar(double valor) {
        if (valor <= 0) {
            throw new MaiorValorException("O valor tem que ser maior que zero");
        }

        if (valor > this.saldo) {
            throw new SemSaldoException("O saldo é insuficiente");
        }
    ...
    public boolean depositar(double valor) {
        ...
        if (isInativa()) {
            throw new InativaException("A conta está inativa");
        }
